...
ength cannot be be longer than 10 (It's obvious, we've got ten digits). 
...
class List(object):
    def count_spec_digits(self, integers_list, digits_list):
        ...
        for i in digits_list:
            if str(i) not in int_str:
                output.append("({}, {})".format(i, 0))
            else:
                output.append("({}, {})".format(i, int_str.count(str(i))))
        
        print(output)

"""
Above solution is a list of strings
['(1, 7)', '(8, 5)', '(4, 0)']
not 
[(1, 7), (8, 5), (4, 0)]

had to have it return a list comp
"""

class List(object):
    def count_spec_digits(self, integers_list, digits_list):
        int_str = ''.join(map(str, integers_list))
        print([(i, int_srt.count(str(i)) for i in digits_list)])
        return [(i, int_srt.count(str(i)) for i in digits_list)]
