...
The number 125874 is the first integer that has this property when the multiplier is 2, thus: 
125874 * 2 = 251748

Make the function search_permMult(), that receives an upper bound, 
nMax and a factor k and will output the amount of pairs bellow nMax that are permuted when an 
integer of this range is multiplied by k. The pair will be counted if the multiple is less than nMax, 
too
...
def search_permMult(nMax, k):
    same = []
    for i in range(1, nMax//k):
        if sorted(list(str(i*k))) == sorted(list(str(i))):
            same.append(i)  
    return len(same)
