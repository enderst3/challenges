...
def travel(r, zipcode):
    street = []
    home = []
    for i in r.split(','):# makes list of strings, split at comma
        if i[-8:] == zipcode:
            i = i[:-8].split()# removes the zipcode and splits
            home.append(i.pop(0))# pops off the first item(house number)
            street.append(' '.join(i))# appends items to street
    return '{}:{}/{}'.format(zipcode, ','.join(street), ','.join(home))#formats answer
