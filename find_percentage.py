...
if __name__ == '__main__':
    ...
    query_name = input()
    for k, v in student_marks.items():# iterate through dict
        if k == query_name: # if k in dict == query 
            score_list = v # set list of scores to variable
    ave_score = (sum(score_list) / len(score_list)) # gets the average of the list
    print("%.2f" % ave_score) # prints to the 100ths 56.00